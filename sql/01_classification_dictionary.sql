/*═══════════════════════════════════════════════════════════════════════════
  ...
  Spec reference : RegTrack_Insights_System_Design_v1.md §6.1 – §6.5
  ...
  WHY THIS EXISTS (do not delete this comment):
    ...
    produced silently wrong numbers on first attempt:
      ...
      • EntitiesAssignment scope is 2-D (branch × category)
...
/*───────────────────────────────────────────────────────────────────────────
  1. VERSION REGISTRY
     ...
     can be explained after the fact (spec §7.2 provenance.dictionary_version).
───────────────────────────────────────────────────────────────────────────*/
...
/*───────────────────────────────────────────────────────────────────────────
  2. STATUS CLASSIFICATION  (spec §6.2)
...
VALUES
 ...
 (1,'ScopeSource','EntitiesAssignment',N'2-D scope: (BranchID × ComplianceCatagoryID). Column is misspelled "Catagory".',N'EntitiesAssignment',N'Chosen over ComplianceCategoryMgmtUser: CM is a strict subset (0 exceptions across 2 tenants) that excludes configured-but-dormant sites, which are themselves a key finding.'),
...
/*───────────────────────────────────────────────────────────────────────────
  6. FAIL-CLOSED COVERAGE CHECK  (spec §6.5)
...
BEGIN
    ...
    SELECT CAST(1 AS BIT) AS StatusCoverageComplete;
END
GO

/*───────────────────────────────────────────────────────────────────────────
  7. CANONICAL OVERDUE PREDICATE  (spec §6.4, Appendix A.5)
...
/*───────────────────────────────────────────────────────────────────────────
  ...
  PROPORTIONATE, mirroring the labelled-placeholder decision (spec §11.4):
