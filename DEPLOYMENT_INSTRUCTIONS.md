...
## 3c. A file can print "installed" while leaving an object DELETED

Every procedure is `DROP` then `CREATE`. If the `CREATE` fails to compile, the
`DROP` has already committed - so the object is **gone**, not merely stale. And
because a failed CREATE does not halt later batches, the trailing
`PRINT '... installed.'` still appears.

This happened on 8 Sep: `sql/03` failed on `usp_Insights_AuditScope` (Msg 130)
and left the procedure deleted from UAT.

**So after any deployment, check the object count as well as the messages:**
...
A procedure short means one failed to create. `98_post_deployment_verification.sql`
checks this, which is why it should be run every time.
