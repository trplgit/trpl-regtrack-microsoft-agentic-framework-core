...
/// declaring it. This class is the fix: the two vendored files are the real thing, fetched
/// once from Google Fonts (OFL-licensed, free to self-host) and checked in - see
/// vendor/README.md for provenance. The render agent's job (per the prompt) is only ever to
/// WRITE THE NAME "Poppins" in font-family/font-family="" - this class supplies the bytes that
/// make that name resolve to something real, post-generation.
/// </summary>
public static class PoppinsFontInjector
{
    ...
    // Loaded and base64-encoded once per process, not per call - these two files never change
    // at runtime (checked-in, CopyToOutputDirectory PreserveNewest), so re-encoding them on
    // every report would just be wasted work on the hot path.
