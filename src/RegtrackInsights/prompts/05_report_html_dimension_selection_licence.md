# Report Generation — Licences, freehand
...
`LicenseTypeName` (real free text, escape it), `IsRetired`, `TotalLicences`, `ActiveLicences`,
`Lapsed` (a not-updated status still counts as lapsed), `ExcludedTerminalState` (ended by
renewal/termination/rejection/not-applicable — NOT a lapse), `LapsingNext30`, `BranchesCovered`,
`LapsedPct`, `OverdueRank`, `Flags`.

**Every real tenant-level total** (`dimension_control_totals`), verbatim: `ScopedLicences`,
`TypedLicences`, `Reconciled`, `TenantLapsedPct`, `LicenceTypesReported`,
`LicenceTypesWithLicences`, `UntypedLicences`, `ExcludedTerminalStateLicences`.

| Fact | Status |
|---|---|
| "{X}% of licences have expired: {N} of {M}" | **REAL** — `TenantLapsedPct` = X; N = `SUM(row.Lapsed)`; M = `ScopedLicences`. Cross-check against the `A-TENANT` assertion's value; if they disagree, use the assertion value and add a data-quality callout. |
| "{E} licences ended for a legitimate reason and are NOT counted as expired" | **REAL** — `ExcludedTerminalStateLicences` = E. Never add these back into the expired count. |
...
- No expiry horizon other than 30 days, no subject-clustering root-cause claim, no consequence
  ranking, excluded-terminal-state licences never merged into the expired count.
