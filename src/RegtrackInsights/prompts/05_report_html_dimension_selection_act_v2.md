# Report Generation — Act & Regulators, freehand (v2, 2026-09-25)
...
## Real vs. NOT AVAILABLE — Act & Regulators dimension
...
**Every real field on an act row** (`dimension_rows`): `ActID`, `ActName`, `RegulatorID` (an id
only — no regulator name is available), `CategoryId`, `Instances`, `Overdue`, `OverduePct`,
`ImprisonmentInstances`, `BranchesCovered`, `StartDate`, `OverdueRank`, `Flags`.
...
| "The most dangerous tasks are the best managed" | **REAL as a pattern, only when the specific rows support it** — a row (or few) with high `ImprisonmentInstances` AND `OverduePct` below `TenantOverduePct`; name the act(s) and quote both real numbers. Never assert it as a general law. |
...
- Every real act row with meaningful volume is represented somewhere — none silently dropped.
- Every number on the page exists in `assertions`, `dimension_rows`, or `dimension_control_totals`.
- No regulator name anywhere, no per-regulator rollup, no subject-clustering root-cause claim.
