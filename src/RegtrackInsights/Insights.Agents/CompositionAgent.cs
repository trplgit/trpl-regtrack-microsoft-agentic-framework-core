...
/// instructions (see CompositionAgentFactory). This class owns only the input/output shape -
...
public sealed class MafCompositionAgent(AIAgent agent) : ICompositionAgent
{
        ...
        CancellationToken cancellationToken = default)
    {
        var payload = JsonSerializer.Serialize(
            new { dimensions = dimensionResults, tenant_shape = tenantShape, report_type = reportType },
            JsonOptions);
        ...
        var message = "Here is the dimension data as JSON:\n" + payload;
